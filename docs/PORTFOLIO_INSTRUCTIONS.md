...
**Purpose:** Display a visual grid of technical skills and technologies with recognizable icons, allowing users to quickly understand the developer's technology stack.

#### 1.7.1 Skills section concept

The Skills section presents a centered grid of technology icons with labels, providing an at-a-glance view of technical proficiencies. Unlike Experience and Projects sections, Skills uses a static grid layout without swipe interactions.

**UX Pattern:**
- Static grid display (no swipe cards)
- Icons arranged in responsive grid
- Hover effects for interactivity
- Centered layout with max-width constraint

**Data Management:**
- Skills stored in `src/data/skills.json` (editable)
- Build script transforms JSON → frozen constant in `src/constants/skills.js`
- Same icon resolution pattern as Projects

**Requirements:**
- Load skill data from generated constants file
- Display skill icons in responsive grid
- Show skill names below icons
- Icons use same `/public/skills/` directory as Projects
- Graceful handling of missing icons

**Acceptance Criteria:**
- [ ] Skills load from `src/constants/skills.js` (auto-generated)
- [ ] Build script runs before dev and build commands
- [ ] Grid is responsive (3 cols mobile → 6 cols desktop)
- [ ] Icons display with hover scale effect
- [ ] Skill names visible below icons
- [ ] Missing icons handled gracefully
- [ ] Content is vertically and horizontally centered

**Dependencies:**
Requires icon infrastructure from 1.5.3 (Projects section)

---

#### 1.7.2 Skill data structure

**Source File:** `src/data/skills.json`
  ...
  "skills": [
    {
      ...
      "icon": string       // Icon filename (no path/extension)
    }
  ]
}
```
...
**Field Descriptions:**
- `id`: Unique numeric identifier for each skill
- `name`: Display name of the technology (e.g., "React", "Node.js")
- `icon`: Icon filename without path or extension (e.g., "react", "nodejs", "typescript-icon")

**Icon Resolution:** Same pattern as Projects
- Looks for `/skills/{icon}-icon.svg` first
- Falls back to `/skills/{icon}.svg`
- Hides if both fail (silent error handling)
- Console warning logged for missing icons
...
**Editing Skills:**
1. Edit `src/data/skills.json` directly
2. Run `npm run build:skills` (or happens automatically with `npm run dev`)
3. Build script regenerates `src/constants/skills.js`
4. Changes reflected in application

---

#### 1.7.3 Build script process

**Purpose:** Transform editable JSON data into immutable JavaScript constants for optimal runtime performance.

**Build Script:** `scripts/build-skills.js`

**Process:**
1. Read `src/data/skills.json`
2. Validate JSON structure (must have `skills` array)
3. Generate `src/constants/skills.js` with frozen export
4. Create output directory if needed
5. Write file with auto-generation warning comment
6. Report success/failure with details
...
**Generated File Format:**
```javascript
// This file is auto-generated by scripts/build-skills.js
...
**Integration:**
- `npm run build:skills` - Run manually
- `npm run dev` - Auto-runs build:skills, then starts dev server
- `npm run build` - Auto-runs build:skills, then production build
- `npm run build:data` - Runs all data build scripts including skills

**Error Handling:**
- Invalid JSON syntax → exit with error message
- Missing `skills` array → validation error
- File read/write errors → descriptive error message
- All errors exit with code 1 (stops build process)
...
#### 1.7.4 Component architecture

**Main Component:**

**SkillsSection.jsx**
- Main container component
- Imports skills from `src/constants/skills.js`
- Renders centered grid of skill items
- Each item: icon + name label
- Icon error handling (same pattern as Projects)
- Hover effects for interactivity
...
└── Grid Container
    └── Skill Items (map over skills array)
        ├── Icon (64px × 64px)
        └── Name Label
```

**Layout Specifications:**
- Container: max-width 4xl (56rem / 896px)
...
- Text: Small (0.875rem), centered, 80% opacity
...
#### 1.7.5 Testing checklist

**Functional Tests:**
- [ ] Grid displays correctly on all screen sizes
- [ ] Icons load and display properly
- [ ] Missing icons are hidden (no broken images)
- [ ] Hover effects work smoothly (scale animation)
- [ ] Skill names are readable in both light and dark themes
- [ ] Section is vertically and horizontally centered
- [ ] Scroll indicators work correctly with Skills section

**Build System:**
- [ ] `npm run build:skills` generates constants/skills.js
- [ ] Generated file has correct structure
- [ ] Build errors are clear and helpful
- [ ] Auto-runs with `npm run dev` and `npm run build`

**Edge Cases:**
- [ ] Icons with alternative naming patterns load correctly
- [ ] Missing icons don't break layout
- [ ] Empty skills array displays gracefully
- [ ] Very long skill names wrap or truncate properly

---

### 1.8 About Section

**Purpose:** Provide comprehensive information about the developer including bio, contact details, education, interests, and downloadable resume.

#### 1.8.1 About section concept

The About section displays a professional profile card with all personal and professional information in a single, scrollable view. Unlike other sections, it uses a static card layout optimized for readability.

**UX Pattern:**
- Single centered card (no swipe)
...
**Data Management:**
- About data stored in `src/data/about.json` (editable)
- Build script transforms JSON → frozen constant
- Resume PDF stored in `/public/resume.pdf`
- Profile image (optional) stored in `/public/`

**Requirements:**
- Display name, title, bio
- Show contact information (email, location, social links)
- List education background
- Display interests as tags
- Provide resume download button
- Optional profile image support
- All links clickable and open in new tabs

**Acceptance Criteria:**
- [ ] About data loads from `src/constants/about.js` (auto-generated)
- [ ] Bio paragraph displays clearly
- [ ] Email link opens mail client
- [ ] LinkedIn/GitHub links open in new tabs with proper security attributes
- [ ] Education section displays if present
- [ ] Interests display as styled tags
- [ ] Download resume button works
- [ ] Profile image displays if provided
- [ ] Content is centered and scrollable
- [ ] Card has consistent styling with portfolio theme
...
**Field Descriptions:**
- `name`: Full name (displayed as main heading)
- `title`: Professional title/role (e.g., "Full Stack Developer")
- `bio`: Personal introduction paragraph (can be multiple sentences)
- `profileImage`: Path to profile photo (e.g., "/profile.jpg") or null
- `contact`: Object containing contact information
  - `email`: Contact email address
  - `location`: Current location/city
  - `linkedin`: Full LinkedIn profile URL
  - `github`: Full GitHub profile URL
- `education`: Array of education entries (optional)
  - `degree`: Degree name
  - `school`: School/university name
  - `year`: Year or year range (e.g., "2016 - 2020")
- `interests`: Array of interest strings (displayed as tags)
- `resumeUrl`: Path to downloadable resume PDF (e.g., "/resume.pdf")
...
```json
{
  "about": {
    ...
    "contact": {
      ...
      "linkedin": "https://linkedin.com/in/carlosestrada",
      "github": "https://github.com/caresle"
    },
    "education": [
      {
    ...
    "interests": ["Open Source", "Web Performance", "UI/UX Design"],
...
**Editing About Data:**
1. Edit `src/data/about.json` directly
2. Run `npm run build:about` (or happens automatically with `npm run dev`)
3. Build script regenerates `src/constants/about.js`
4. Changes reflected in application

**Assets:**
- Resume: Place PDF at `/public/resume.pdf`
- Profile Image: Place image at `/public/profile.jpg` (or .png)
- Update JSON paths accordingly

---

#### 1.8.3 Build script process

**Purpose:** Transform editable JSON data into immutable JavaScript constants.

**Build Script:** `scripts/build-about.js`

**Process:**
1. Read `src/data/about.json`
2. Validate JSON structure (must have `about` object)
3. Generate `src/constants/about.js` with frozen export
4. Create output directory if needed
5. Write file with auto-generation warning comment
...
**Generated File Format:**
```javascript
// This file is auto-generated by scripts/build-about.js
...
**Integration:**
- `npm run build:about` - Run manually
- `npm run dev` - Auto-runs build:about, then starts dev server
- `npm run build` - Auto-runs build:about, then production build
- `npm run build:data` - Runs all data build scripts including about

**Error Handling:**
- Invalid JSON syntax → exit with error message
- Missing `about` object → validation error
- Type mismatch → validation error
- File read/write errors → descriptive error message

---

#### 1.8.4 Component architecture

**Main Component:**

**AboutSection.jsx**
- Main container component
- Imports about from `src/constants/about.js`
- Renders scrollable card with all sections
- Sections: Header → Bio → Contact → Education → Interests → Resume
...
└── Scrollable Container
    └── White Card
        ...
        ├── Interests Section (conditional)
        └── Download Resume Button (conditional)
```

**Layout Specifications:**
- Container: max-width 3xl (48rem / 768px)
- Card: White background, rounded-2xl, shadow-lg, padding 10
- Profile Image: 128px × 128px, rounded-full (if provided)
- Title Sizes: h1 (4xl), h2 (2xl)
- Spacing: mb-8 between sections
- Interests: Gray background tags, rounded-full
- Resume Button: Black bg, white text, rounded-xl, centered
...
#### 1.8.5 Testing checklist

**Functional Tests:**
- [ ] Card displays centered and styled correctly
- [ ] Bio text is readable and wraps properly
- [ ] Profile image displays if provided (or section hidden if null)
- [ ] Email link opens mail client (`mailto:` protocol)
- [ ] LinkedIn link opens in new tab with correct URL
- [ ] GitHub link opens in new tab with correct URL
- [ ] Education section displays if present
- [ ] Interests display as styled tags
- [ ] Download resume button downloads PDF file
- [ ] Content is scrollable if longer than viewport

**Visual Tests:**
- [ ] Card styling matches portfolio theme (white card, consistent colors)
...
**Responsive Tests:**
- [ ] Card width adjusts on small screens
- [ ] Content doesn't overflow horizontally
- [ ] Tags wrap on mobile
- [ ] Button remains centered

**Build System:**
- [ ] `npm run build:about` generates constants/about.js
- [ ] Generated file has correct structure
- [ ] Build errors are clear and helpful
- [ ] Auto-runs with `npm run dev` and `npm run build`
